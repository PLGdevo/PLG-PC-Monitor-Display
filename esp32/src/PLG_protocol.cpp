...
// Xu ly 1 dong da hoan chinh (da co '\0' o cuoi serial_line_buf).
static void protocol_handle_line(ProtocolReplyFn reply)
{
    // Bat ky dong nao doc duoc trot lot (ke ca bat tay PLG_ID?) deu tinh la "co tin hieu song"
    // tu PC - khong doi den khi parse ra CPU/RAM... moi coi la connected.
    last_rx_ms = millis();
    has_rx_ever = true;

    // Bat tay nhan dien thiet bi: monitor.py gui "PLG_ID?" khi do cong tu dong, board tra loi
    // cau co dinh nay de PC xac nhan dung la board PLG (khong phai thiet bi USB Serial/BLE khac)
    // ma khong can nguoi dung tu chon cong.
    if (strcmp(serial_line_buf, "PLG_ID?") == 0)
    {
        if (reply != nullptr)
            reply("I AM PLG_TFT_LCD_TASKMANAGER\n");
        return;
    }
    ...
    // TEMP la truong moi them sau (monitor.py cu chua gui): chi push khi co mat, khong thi giu
    // nguyen bieu do nhiet do o trang thai "chua co du lieu" (-1)
    if (n >= 6)
    {
