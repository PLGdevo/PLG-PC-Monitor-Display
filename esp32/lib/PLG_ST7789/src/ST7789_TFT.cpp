...
{
	// ESP32: SPI.begin() vua cap xung vua gan chan vao ngoai vi SPI (thay cho spi_init +
	// gpio_set_function cua pico-sdk). Man hinh chi ghi, khong doc -> MISO = -1.
	// CS do thu vien tu dieu khien bang digitalWrite nen cung truyen -1.
	_pspiInterface->begin(_TFT_SCLK, -1, _TFT_SDATA, -1);
	// Che do MODE0 (CPOL=0, CPHA=0), MSB truoc - giong cau hinh spi_set_format cu.
	_pspiInterface->beginTransaction(SPISettings((uint32_t)_speedSPIKHz * 1000, MSBFIRST, SPI_MODE0));
}
...
void ST7789_TFT ::TFTPowerDown(void)
{
	...
	if (_hardwareSPI == true) {
		_pspiInterface->endTransaction(); // dong transaction mo trong TFTHWSPIInitialize
		_pspiInterface->end();
	}else{
		TFT_SCLK_SetLow;
		TFT_SDATA_SetLow;
...
/*!
	...
	@param spi_interface Spi interface, tren ESP32-S3 la &SPI (FSPI) hoac &SPI1
	@note method overload used , method 1 hardware SPI
	@note _speedSPIKHz la uint16_t nen gia tri toi da la 65535 (65 MHz) - truyen lon hon
	      se bi tran (wrap) va cho ra toc do SPI sai, khong phai bi kep xuong tran.
*/
void ST7789_TFT  :: TFTInitSPIType(uint32_t speed_Khz,  SPIClass* spi_interface)
{
