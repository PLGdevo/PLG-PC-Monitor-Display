...
void MONITOR_BEGIN()
{
	...
	// the sang phia sau logo de logo (nen den) khong bi chim vao man hinh
	myTFT.TFTfillRoundRect(85, 35, 150, 130, 8, UI_PANEL);
	myTFT.TFTdrawRoundRect(85, 35, 150, 130, 8, UI_BORDER);
	drawLogoFull(100, 50, 120, 100, PLG_logo);

	char title[16] = "TASK MANAGER";
	myTFT.TFTdrawText(70, 185, title, UI_ACCENT, UI_BG, 2);

	int16_t barX = 60, barY = 225, barW = 200, barH = 12;
	myTFT.TFTdrawRoundRect(barX, barY, barW, barH, 4, UI_BORDER);

	for (int a = 0; a <= 100; a++)
	{
		char load[5] = "";
		snprintf(load, sizeof(load), "%d%%", a);
		myTFT.TFTfillRect(90, 250, 60, 20, UI_BG);
		myTFT.TFTdrawText(90, 250, load, UI_TEXT, UI_BG, 3);
...
void draw_battery(int16_t iconX, int16_t textX, int8_t &value, int8_t &lastValue)
{
	...
	uint16_t col = (value <= 20) ? UI_DANGER : UI_ACCENT;
...
serial_line_buf[serial_line_len] = '\0';
				int cpu = -1, ram = -1, gpu = -1, wifi = -1, bat = -1;
				char time_buf[9] = "", date_buf[11] = "";
				int n = sscanf(serial_line_buf, "CPU:%d;RAM:%d;GPU:%d;WIFI:%d;TIME:%8[^;];DATE:%10[^;];BAT:%d", &cpu, &ram, &gpu, &wifi, time_buf, date_buf, &bat);
				if (n >= 4)
				{
					  ...
					  int warnAt = 101, const char *warnText = nullptr)
{
	...
	uint16_t textCol = warn ? UI_DANGER : UI_TEXT;
...
// vi tri 4 the chart, tinh 1 lan dung chung cho ve khung lan dau va cap nhat du lieu
struct ChartLayout
{
	int16_t x, y, w, h;
};
void get_chart_layout(ChartLayout out[4])
{
	const int16_t fullW = 300, topH = 92, botH = 54;
	...
	out[2] = {startX, (int16_t)(startY + topH + gapY), fullW, botH};	  // GPU
	out[3] = {startX, (int16_t)(startY + topH + gapY + botH + gapY), fullW, botH}; // WIFI
}
...
void MONITOR_TASKMANAGER()
{
	...
	if (taskmanager_dirty)
	{
		...
		draw_chart_data(c[3].x, c[3].y, c[3].w, c[3].h, "WIFI", chart_wifi, UI_WIFI);
...
void MONITOR_CLOCK()
{
	...
	if (clock_dirty)
	{
		if (strcmp(last_time_str, current_time_str) != 0)
		{
			...
			myTFT.TFTdrawText(40, 105, current_time_str, UI_ACCENT, UI_BG, 5);
